    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            for assignment in obsVarFactor.getAllPossibleAssignmentDicts():
                prob = 0
                if houseVal != assignment[GHOST_HOUSE_VAR] and houseVal != assignment[FOOD_HOUSE_VAR]:
                    if assignment[obsVar] == NO_OBS_VAL:
                        prob = 1
                    else:
                        prob = 0
                elif houseVal == assignment[FOOD_HOUSE_VAR]:
                    if assignment[obsVar] == RED_OBS_VAL:
                        ...
                        prob = 0
                elif houseVal == assignment[GHOST_HOUSE_VAR]:
                    if assignment[obsVar] == RED_OBS_VAL:
                        prob = PROB_GHOST_RED
                    elif assignment[obsVar] == BLUE_OBS_VAL:
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    highestProb = -sys.float_info.max
    mostLikelyPosition = dict()
    for assignment in factor.getAllPossibleAssignmentDicts():
        prob = factor.getProbability(assignment)
        if prob > highestProb:
            highestProb = prob
            mostLikelyPosition = assignment
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        probFoodhouseTopleft = 0
        probFoodhouseTopright = 0
        for assignment in factor.getAllPossibleAssignmentDicts():
            if assignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                probFoodhouseTopleft = factor.getProbability(assignment)
            elif assignment[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                probFoodhouseTopright = factor.getProbability(assignment)

        leftExpectedValue = probFoodhouseTopleft * WON_GAME_REWARD + (1 - probFoodhouseTopleft) * GHOST_COLLISION_REWARD
        rightExpectedValue = probFoodhouseTopright * WON_GAME_REWARD + (1 - probFoodhouseTopright) * GHOST_COLLISION_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for (prob, explorationEvidence) in self.getExplorationProbsAndOutcomes(evidence):
            leftExpected, rightExpected = self.computeEnterValues(explorationEvidence, enterEliminationOrder)
            expectedValue = expectedValue + prob * max(leftExpected, rightExpected)
