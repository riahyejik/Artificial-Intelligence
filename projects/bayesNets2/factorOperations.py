...
def joinFactors(factors):
    ...
    for factor in factors:
        ...
        for uncondVar in factor.unconditionedVariables():
            unconditioned.add(uncondVar)

    for condVar in temp:
    	if condVar not in unconditioned:
    		conditioned.add(condVar)

    newFactor = Factor(unconditioned, conditioned, factors[0].variableDomainsDict())
    for assignment in newFactor.getAllPossibleAssignmentDicts():
    	prob = 1
    	for factor in factors:
    		prob = prob * Factor.getProbability(factor, assignment)
    ...
    def eliminate(factor, eliminationVariable):
        ...
        for assignment in newFactor.getAllPossibleAssignmentDicts():
            ...
            for elimVar in factor.variableDomainsDict()[eliminationVariable]:
                temp = assignment
                temp[eliminationVariable] = elimVar
                prob = prob + factor.getProbability(temp)
...
def normalize(factor):
    ...
    for assignment in factor.getAllPossibleAssignmentDicts():
        probSum = probSum + factor.getProbability(assignment)
    if probSum == 0.0:
        return None
    ...
    for assignment in newFactor.getAllPossibleAssignmentDicts():
        prob = factor.getProbability(assignment) / probSum
        newFactor.setProbability(assignment, prob)
    return newFactor
