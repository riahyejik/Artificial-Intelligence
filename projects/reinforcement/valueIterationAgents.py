...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        "*** YOUR CODE HERE ***"
        sum = 0
        for nextS, trans_prob in self.mdp.getTransitionStatesAndProbs(state, action):
        	sum += trans_prob*(self.mdp.getReward(state, action, nextS) + 
        		self.discount*self.values[nextS])
        return sum
    ...
    def runValueIteration(self):
        ...
        for state in states:
        	if not self.mdp.isTerminal(state):
        		for act in self.mdp.getPossibleActions(state):
        			for nextS, trans_prob in self.mdp.getTransitionStatesAndProbs(state, act):
        				if trans_prob > 0.0:
        ...
        for i in range(self.iterations):
        	...
        	for p in predecessors[state]:
        		...
        		for act in self.mdp.getPossibleActions(p):
        			q = self.computeQValueFromValues(p,act)
        			if q > max_q_value:
        				max_q_value = q
