...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        mDistance = sys.float_info.max
        for liveGhost in livingGhostPositionDistributions:
        	temp = self.distancer.getDistance(pacmanPosition,liveGhost.argMax())
        	if temp < mDistance:
        		mDistance = temp
        		mostLikelyPosition = liveGhost.argMax()
        action = None
        mDistance = sys.float_info.max
        for act in legal:
        	successorPosition = Actions.getSuccessor(pacmanPosition, act)
        	temp = self.distancer.getDistance(successorPosition, mostLikelyPosition)
        	if temp < mDistance:
        		mDistance = temp
        		action = act
        return action
