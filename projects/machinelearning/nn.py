...
class Graph(object):
    ...
    def backprop(self):
        ...
        for node in reversed(self.graph):
            outputs = node.backward(self.get_inputs(node), node.gradient)
            for i in range(len(node.get_parents())):
                node.get_parents()[i].gradient = np.add(node.get_parents()[i].gradient,
                                                        outputs[i])
    ...
    def step(self, step_size):
        ...
        "*** YOUR CODE HERE ***"
        for var in self.variables:
            var.data = np.subtract(var.data, step_size * var.gradient)
...
class MatrixMultiply(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        df_dA = np.dot(gradient, inputs[1].T)
        df_dB = np.dot(inputs[0].T, gradient)
        return [df_dA, df_dB]
